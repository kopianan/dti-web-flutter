...
@RoutePage()
class AdminApplicationDraftPage extends StatefulWidget {
  const AdminApplicationDraftPage({super.key});

  @override
  State<AdminApplicationDraftPage> createState() => _AdminApplicationDraftPageState();
}

class _AdminApplicationDraftPageState extends State<AdminApplicationDraftPage> {
  @override
  void initState() {
    context.read<AdminApplicationCubit>().getAllUserVisaDraft();
    super.initState();
  }
    ...
    return BlocListener<AdminApplicationCubit, AdminApplicationState>(
        listener: (context, state) {
          state.maybeMap(
            ...
            getAllUserVisaDraft: (e) {
              EasyLoading.dismiss();
              context.read<AdminDataCubit>().setApplicationDraftData(e.visas);
            },
            loading: (e) {
              EasyLoading.show();
            },
                  ...
                  child: BlocBuilder<AdminDataCubit, AdminDataState>(
                    builder: (context, adminState) {
                      return DataTable2(
                        ...
                        rows: adminState
                            .getListApplicationDraft()
                            .map((value) => applicationDataRow(value,
                                adminState.getListApplicationDraft().indexOf(value)))
                            .toList(),
                      );
                    },
                  ),
